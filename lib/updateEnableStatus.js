 ...
 * @param {Date} now - JST時刻
 * @param {object} options - オプション（例: { forceOverride: true }）
 */
function updateEnableShots(wallet, now, options = {}) {
  ...
  function buildJST(date, h) {
    const jstDateStr = date.toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' });
    const jst = new Date(jstDateStr);
    jst.setHours(h === 0 ? 0 : h, 0, 0, 0);
    return jst;
  }
