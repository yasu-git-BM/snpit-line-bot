...
function updateEnableShots(wallet, nowJST, options = {}) {
  ...
  for (const h of recoveryHours) {
    ...
    if (!recovered && lastChecked < boundary && now >= boundary) {
	  ...
	  console.log(`🔍 ウォレット名 : ${wallet['wallet name']} 【エネ回復】カメラ撮影可能枚数 : ${enableShots} lastChecked(${new Date(lastChecked).toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' })}) < boundary(${new Date(boundary).toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' })}) && now(${new Date(now).toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo' })}) >= boundary(${new Date(boundary).toLocaleString('ja-JP', { timeZone: 'Asia/Tokyo })})`);
      recovered = true;
    }
  }
