...
function updateEnableShots(wallet, nowJST, options = {}) {
  ...
  console.log(`### updateEnableShots() START`);
  ...
  console.log(`### updateEnableShots() maxShots=${maxShots} / options.forceOverride=${options.forceOverride} / wallet.manualOverride=${wallet.manualOverride} / options.ignoreManual=${options.ignoreManual} `);
  ...
  if (options.forceOverride === true) {
    ...
    wallet.enableShots = enableShots;
     console.log(`### updateEnableShots() SKIP END options.forceOverride === true`);
    return;
  }

  if (wallet.manualOverride === true && options.ignoreManual !== true) {
    wallet.manualOverride = false;
     console.log(`### updateEnableShots() SKIP END wallet.manualOverride === true && options.ignoreManual !== true`);
    return;
  }
  ...
  console.log(`🔍 ウォレット名 : ${wallet['wallet name']} 【更新後】最終カメラ撮影可能枚数 : ${enableShots}`);
     console.log(`### updateEnableShots() NORMAL END`);
}
