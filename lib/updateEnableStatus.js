...
function updateEnableShots(wallet, nowJST, options = {}) {
  ...
  const now = typeof nowJST === 'string'
    ? DateTime.fromISO(nowJST, { zone: 'Asia/Tokyo' })
    : DateTime.fromJSDate(nowJST, { zone: 'Asia/Tokyo' });
  ...
  for (const h of recoveryHours) {
    ...
    console.log(`🕒 回復判定: lastChecked(${lastChecked.toFormat('yyyy/MM/dd HH:mm:ss')})[${lastCheckedMinutes}] < boundary(${boundary.toFormat('yyyy/MM/dd HH:mm:ss')})[${boundaryMinutes}] && now(${now.toFormat('yyyy/MM/dd HH:mm:ss')})[${nowMinutes}] >= boundary(${boundary.toFormat('yyyy/MM/dd HH:mm:ss')})[${boundaryMinutes}]`);
